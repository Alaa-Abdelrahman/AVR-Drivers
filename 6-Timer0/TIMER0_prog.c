...
ErrorState_t TIMER0_SetBusyWait(uint64_t Period){
	...
	ErrorState_t state = DONE;
	uint32_t TickTime,OverFlowTicks,Freq,Prescaler,DesiredTicks;
 ...
 * Function: TIMER0_ReadTimerReg()
  ...
  *
  * @param[OUT]  value of register
  *
  * @return Error state
 ******************************************************************************/
ErrorState_t TIMER0_ReadTimerReg(uint32_t *Value){
	ErrorState_t state = DONE;
	if(Value !=NULL){
		*Value = TCNT0;
	}
